...
import {
  Plus, DollarSign, Wrench, Clock, Download, Trash2,
  RefreshCw, CreditCard, MessageSquare, Mail, ArrowLeft,
} from "lucide-react";
...
import { useCustomerTickets, useExportTickets, useDeleteTicket } from "@/hooks/use-api";
...
function CustomerTicketsContent() {
  ...
  const customerLabel = emailParam || nameParam || "Customer";

  const filter = emailParam ? { email: emailParam } : { name: nameParam };
  ...
  const { data: apiTickets, isLoading } = useCustomerTickets(filter, branchId, {
    special_order: specialOrders ? "Yes" : undefined,
  });

  const tickets: ApiTicket[] = (apiTickets || []) as unknown as ApiTicket[];
  ...
  return (
    <PageShell>
      <PageHeader
        title={`Tickets for ${customerLabel}`}
        description={`${branch} — ${tickets.length} tickets found`}
        actions={
          <div className="flex items-center gap-2">
            <Link href="/customers">
              <Button variant="outline" size="sm"><ArrowLeft className="h-3.5 w-3.5 mr-1.5" />Back to Customers</Button>
            </Link>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <KpiCard title="Total Revenue" value={`$${totalRevenue.toFixed(0)}`} icon={DollarSign} />
      </div>
...
export default function CustomerTicketsPage() {
  return (
    <Suspense fallback={<BrandedLoader variant="page" text="Loading customer tickets..." />}>
